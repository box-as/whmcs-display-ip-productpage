        ...
        <tbody>
            {foreach key=num item=service from=$services}
                <tr onclick="clickableSafeRedirect(event, 'clientarea.php?action=productdetails&amp;id={$service.id}', false)">
                    ...
                    <td><strong>{$service.product}</strong>{if $service.domain}<br /><a href="http://{$service.domain}" target="_blank">{$service.domain}</a>{/if} - {if $service.dedicatedip}<br />{$service.dedicatedip}{/if}</td>   {* Just Mofidy this line with "{if $service.dedicatedip}<br />{$service.dedicatedip}{/if}" *}
